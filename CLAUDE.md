...
├── deck/HtmlImageLibrary.tsx  # 043：デッキの HTML 画像ライブラリ（SqlInitModal の footer に差し込む。追加/リネーム/タグコピー/削除）
...
**HTML 画像ライブラリ（043・Pro 限定）**：デッキに写真を登録し、HTML ブロック／HTML 土台から **`<img src="img://名前">`**（CSS の `url(img://名前)` でも可）で参照する。`decks.htmlImages`（`DeckImage[] = {name, uri}` の JSON 列）に**参照名とパスだけ**を保存し、画像の実体は画像ブロックと同じ `images/`（`local://images/xxx`）に置く。**base64 は実行の瞬間にメモリ上で作るだけで DB には入らない**（＝エクスポート・iCloud 同期・エディタの入力欄が膨らまない。これが data URI 直貼りに対する本方式の存在理由）。解決は `lib/htmlImages.ts` の `resolveHtmlImageRefs()` が **合成後の最終 HTML に対して1回**行い、`sandbox.ts` は無改造（`img://` は土台にも本文にも書けるため最終形に1回かければ両方拾える）。差し込み口は3つ＝`useCodeExecution.run()`（実行）・`ExecutionOutput`（実行前プレビュー・**400msデバウンスの後**で解決）・`InteractivePreviewModal`（041・**解決待ちの間は WebView を出さない**＝未解決 HTML を描画して差し替えるとスクリプトが2回走るため）。`hasImageRefs()` の同期判定で **`img://` を含まないカードは従来どおり完全に同期**（挙動・速度とも不変）、かつ **web 系限定**（python/sql 本文の `img://` は書き換えない）。非同期化に伴い `runSeqRef` の通し番号で「解決待ちの間に次の実行/clear が走ったら古い結果を捨てる」。登録は `pickAndSaveImage({ maxDimension })`＝長辺1024へ縮小＋png/jpg へ正規化（HEIC 対策・MIME 判定を2択に固定）。UI は `HtmlImageLibrary`（`SqlInitModal` の `footer` prop に差し込む・既定は折りたたみ）。**削除は配列から外すだけでファイルは消さない**（保存前キャンセルで参照が生き残るため。孤児は起動時掃除が回収）。解決できない参照は**プレースホルダ SVG**（`img://name` を描画）になる。
