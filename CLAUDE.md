...
**デッキ土台の複数持ち（044・Pro 限定）**：デッキは**名前付きの HTML/CSS 土台を複数**持てる（`decks.htmlStages`＝`DeckStage[] = {id,name,html}` の JSON 列）。コードブロックは `CodeBlock.deckStageId` で1つを選ぶ。**読み取りの正は `htmlStages` だけ**で、旧 `decks.htmlInit` は**先頭土台のミラー**として残してある（旧バージョンのアプリは `htmlInit` しか読まない＝iCloud は DB ファイルごと往復するため。書き戻しは `legacyHtmlInitMirror()`）。旧データの吸収は **`toDeck` の `normalizeDeckStages()` に閉じている**（`htmlStages` が空なら `htmlInit` から1件合成・合成した土台の id は定数 `LEGACY_STAGE_ID`＝毎回 `generateId()` すると `deckStageId` の照合と React の key が揺れる）ので、**画面・コンポーネントは `Deck.htmlStages` だけを見ればよい**。土台の解決は `lib/deckStages.ts` の **`resolveDeckStageHtml(stages, block)`**（`noDeckHtmlInit`→積まない／`deckStageId` 未指定→**先頭**＝044 以前の挙動／**解決できない id（削除された土台）→積まない**）。⚠️ **解決できないときに先頭へフォールバックしてはいけない**＝別の土台が黙って適用されて出題の前提が変わるため。土台なしなら js/ts はコンソール実行に戻り html は本文だけになるので作者が気づける。UI は デッキ編集の「HTML/CSS 共通土台」行／`H` キー →`DeckStagesModal`（一覧＝追加/削除/1行プレビュー・先頭に「既定」バッジ・行タップで `SqlInitModal` が開き**名前はそのタイトル欄で編集**）。ブロック側は **土台が1つなら従来の ON/OFF トグルのまま、2つ以上のときだけ「使わない＋各土台」のチップ選択**に変わる（＝土台を1つしか使わないデッキは 044 以前と見た目も操作も同じ）。**中身が空の土台は保存しない**。TSV は往復しない（`inspectTsvExport` は件数を数えて警告に出す）。詳細は `docs/044`。
