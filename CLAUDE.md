...
**実行中ログの逐次表示（console・040 インライン共通）**：ログは終了時の1通（`finish` の `logs`）が正だが、それだけだと `setInterval(fn, 1000)` のような「時間をかけて出力するコード」の途中経過が見えない（締切に達してから一気に出る）。`sandbox.ts` の `LOG_STREAM_SCRIPT` が `console.*` のたびにキューへ積み、**100ms ごとに `{type:'logs', entries}` でまとめて送る**（`useCodeExecution` が `liveLogs` に溜め、`ExecutionOutput` が実行中だけ表示。`result` には入れない＝`result` の変化は `onResult`＝学習画面の結果へのスクロールを呼ぶため）。**逐次送信は `LOG_STREAM_MAX`（200件）まで**で、超過分は終了時の1通に入る＝最終的な表示内容は不変。**送信予約は `_origSetTimeout`（ラップ前）で行う**（ラップ後だと保留タイマーに数えられ完了判定が遅れ、締切まで延ばしてしまう）。100ms 以内に完了する普通のコードは `flushLogs` が `_done` で弾かれるため**逐次送信そのものが発生しない**（＝従来と完全に同じ1通のみ）。UI は**最初のログが届いてから枠を出す**方式（案A）で、ログを出さないカード（プレビューだけの html/css 等）の実行中の見た目は変えない。
