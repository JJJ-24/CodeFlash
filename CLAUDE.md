...
- **デッキ新規作成キー**: N = デッキ名にカーソル、M = 説明欄にカーソル（Memo/Message。N と隣接）、C = カラー順送り（青→プリセット→テーマ色→白黒の循環。Shift+C で逆順）、I = アイコン選択を開く、Q = SQL初期化の一覧を開く（Pro時）、H = HTML/CSS 土台の一覧を開く（Pro時）、U/D・PgUp/PgDn = 画面スクロール（上/下・段階）、Home/End = 最上部/最下部、S = 保存、X = 閉じる、Esc = 編集中→カーソル解除／非編集→閉じる。デッキ名 Return で説明欄へ移動。Tab/矢印は不使用（iPad フォーカスエンジン対策＝文字キー＋住み分けで完結）
