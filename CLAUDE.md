...
├── deck/HtmlImageLibrary.tsx  # 043：デッキの HTML 画像ライブラリ（DeckStagesModal の listFooter＝土台一覧の末尾**だけ**に差し込む。追加/リネーム/タグコピー/削除）
...
実行できる言語は `EXECUTABLE_LANGUAGES`（`javascript`・`typescript`・`python`・`sql`・`cpp`・`html`・`css`）で、うち `PRO_LANGUAGES`（`sql`・`cpp`・`html`・`css`）は Pro 限定。**C++ だけは WebView サンドボックスを使わない例外**で、`runCppViaWandbox()` が Wandbox の公開 API（`https://wandbox.org/api/compile.json`・gcc-13.2.0 / `-std=c++17`）に POST して結果を受け取る。ネットワーク実行のため固有の事情がある：全体30秒の `AbortController` タイムアウト（超過で `status: 'timeout'`）、公開インスタンスの混雑（`WANDBOX_TRANSIENT_PATTERN` に一致する一時障害）は 800ms→1600ms のバックオフで自動リトライし、それでも復旧しなければコードの誤りと区別して `code.serverBusy` を案内する。コンパイルエラー（`compiler_error` に `error:` を含む）はユーザーのコードの問題なのでリトライ対象にしない。

**HTML/CSS プレビュー実行（040・Pro 限定）**：`html` ブロックは本文（HTML/CSS/JS）をそのまま描画、**`css` ブロックは本文（CSS）を `<style>` に入れて土台を装飾**（2026-07-29 追加。**土台が無くても常に web 実行**＝html と同じ扱い。js/ts と違い「コンソール実行」という落とし先が無いため。⚠️ **土台側で `style` 属性を使うと詳細度 1,0,0,0 で css ブロックから上書きできず、しかも CSS はエラーを出さないので「実行しても何も起きない」になる → 土台の基本スタイルは `<style>` に書く**）、`javascript`/`typescript` ブロックは **HTML/CSS 土台**（`Deck.htmlInit`＝デッキ共通 ＋ `CodeBlock.htmlInit`＝ブロック固有）を JS で操作する。土台は SQL 初期化と同じ加算型（デッキ→ブロック→本文）で、**ブロック固有土台は web 系4言語すべてが持てる**（html も 2026-08-08 から。当初は「本文が主役」として html だけ除外していたが、それだと html だけ**カード単位の「出題の前提」を置けない**＝デッキ全体でしか前提を設定できず、`previewInit` は「本文＝答えを見せる」機能なので代替にならないため対称化した。土台＝実行前から見える前提／本文＝実行して初めて出る答え、という軸は4言語共通）。**加算であって差し替えではない**：デッキ土台・ブロック土台・本文に `<style>` があれば `<style>` 要素が3つ並び CSS のカスケードで後勝ち（本文＞ブロック＞デッキ）になる。アプリ側で畳んで1つに見せる案は**不採用**＝①任意の HTML 断片に対する置き換えの単位を定義できずセレクタ単位のマージはカスケードの再実装になる ②置き換えたいときは `noDeckHtmlInit`（ブロック単位・明示的）が既にある ③カスケードそのものが教材で、畳むと実物と違う挙動を教えることになる。⚠️ **「後勝ち」は詳細度が同じときだけ**（土台の `style` 属性・`!important` には勝てない）／⚠️ **DOM は追記だと重複する**（デッキ土台とブロック土台に同じ `id` があると要素が2つ並び `getElementById` は先頭＝デッキ側を返す）＝**DOM を差し替えたい場面では `noDeckHtmlInit` を切る**。詳細は `docs/040`。実行系は `buildWebSandboxHtml()`＝`<head>` にネットワーク遮断・console キャプチャ・保留タイマー追跡を置き、インライン `<script>` の未捕捉例外は `window.onerror`、完了判定は `DOMContentLoaded` 後（後出しログはマクロタスク境界まで待つ・全体の締切は既定5秒＝下記の延長あり）。`ExecutionOutput` は web 系のとき **可視 WebView**（`pointerEvents="none"`・`onInteract` で学習画面のタップ時に `suppress()` してフリップ抑止）で描画し `[プレビュー | ソース]` トグルを出す（**ソースは実行前＝土台テキスト／実行後＝`formatBodyHtml()`（body を整形して文字列化）**。`finish()` が `{type:'resultSource'}` で送り `ExecutionOutput` が横取り＝`useCodeExecution` は無改造。base64 は中身を省略し 100KB で打ち切る。**整形は `HTML_FORMAT_SCRIPT` が DOM を歩いて行う**：`outerHTML` は「書いたときの改行をそのまま返す」だけなので、`<script>` の中身がタグと同じ行に詰まり、`appendChild` で JS が作った要素は改行テキストが無く全部1行に連なって読めない。ルールは ①空白だけのテキストノードは捨てる ②子が全部インライン要素/テキストなら1行に畳む（200文字まで） ③`<script>`/`<style>` はタグと中身を別行にし、中身は `_reindent()`（最小共通インデントを外してタグの1段下へ揃え直す＝相対的な段差は保つ）。原文のまま出す案は、土台の書き方次第でタグと中身の位置関係が崩れる（字下げ無しで書くとタグより左に出る）ため不採用。**複数行のテンプレートリテラルは表示上だけ空白が増える**（実行・プレビューには無影響） ④`<pre>`/`<textarea>` と `white-space: pre*`（`getComputedStyle` で判定）は中身に触れない ⑤属性値は `<` `>` も escape（ハイライトがタグ開始と誤認するため）。**表示専用で再実行しないので、整形による空白の変化は実行に影響しない**。⟲/再実行で捨てて土台テキストへ戻る＝プレビューと同じ「実行前/実行後」の軸に乗る。土台も結果も無ければソースタブ自体を出さない）。未実行の js/ts・**html**＋土台は `buildStaticPreviewHtml()`（土台のみ・表示専用）の**実行前プレビュー**を自動表示（土台編集は400msデバウンス）。html を含めるのは「土台に追記して完成させる」出題のため（当初は対象外だったが 2026-07-29 に変更）。土台が無いカードは枠自体が出ないので従来どおり。**html だけ `CodeBlock.previewInit`（JSON の任意フラグ・マイグレーション不要・既定 false）で本文も実行前に描ける**：ON で `staticBody` を渡し「実行前プレビュー＝土台＋本文」になる。常時ONにしないのは「この HTML はどう表示される？」と**予想させる出題の答えが先に見えてしまう**ため。**js/ts には `staticBody` を渡さない**（本文が JS で実行前に走らせてはいけない）。**`CodeBlock.noDeckHtmlInit`（JSON の任意フラグ・マイグレーション不要・既定 false＝積む）でブロック単位にデッキ共通土台を切れる**（2026-07-30 追加）：web 系4言語すべて対象で、ON にすると `Deck.htmlInit` を積まない（ブロック固有 `htmlInit` は残る）。js/ts は土台が全部空になると `hasStage` false ＝**コンソール実行に戻り実行前プレビューも⛶も消える**＝「このカードは console.log を見るだけ」の出題で無関係なプレビューを出さないための機能。カード単位（`cards` 列追加）にしなかったのは、1枚にコンソール専用ブロックと DOM 操作ブロックを混在させられるうえ、土台を消費しているのがブロックだから。実装は `CodeRunnerView`/`CodeBlockItem` の `htmlInits` 組み立てに `deckStage` を挟むだけ（下流は全部 `htmlInits` 派生なので自動追従・サンドボックスは無改造）。UI は `previewInit` と同型のトグル行で、**ラベルは肯定形「デッキ共通の土台を使う」＝既定 ON・保存値は否定形**、表示は web 系＋Pro＋デッキに土台があるときのみ。学習画面は保存値を使い学習者の編集は反映しない＝「実行前＝問題の初期状態／実行＝自分の書き足しを反映」の区別を保つ（編集画面では打鍵追従＝オーサリング支援）、実行結果表示中は右端の⟲リセットで初期状態へ戻す。同一コードの再実行で完了メッセージが来ず固着するのを防ぐため、WebView は `runNonce` を key にして毎回再マウントする。Pro ゲートは言語で方式が異なる：html/css は `PRO_LANGUAGES`（実行ゲート＝実行ボタンで Pro モーダル）、**js/ts は「土台を積むかどうか」でゲートする**＝`CodeRunnerView`/`CodeBlockItem` の `htmlInits` 組み立てで**非 Pro なら `deckStage`/`blockStage` を空にする**（`noDeckHtmlInit` と同じ経路なので下流は無改造）。土台が空になれば `hasStage` false でコンソール実行に落ち、`previewSource` も空になるので実行前プレビュー・ソースタブ・⛶ が自動的に消える（入力欄と `canStaticPreview`/`canExpand` の `isPro` と二重に塞がる）。**この「積まない」処理は必須**：かつては `run()` に `htmlInits` を `isPro` 関係なく渡していたため、土台のあるデッキでは非 Pro でも js/ts の実行で可視プレビューと実行後 DOM のソースタブが見えていた（土台は体験終了後の端末や JSON/TSV インポート・iCloud 経由でも入るので、土台つきデッキを配れば無料版で Web プレビューが使えた）。⚠️ **js/ts の実行そのものはブロックしてはいけない**：JS 実行は無料機能で、デッキ共通土台は既定 ON かつ切る手段（`noDeckHtmlInit` トグル）が Pro 限定のため、止めると「土台のあるデッキでは `console.log` すら実行できない」逃げ場のない状態になる（実装途中で実際にそうなり差し替えた）。代わりに `ExecutionOutput` の `proStageHint`（`code.proStageHint`）で結果パネルの末尾に「土台なしで実行した」旨を1行出す＝DOM を触るコードはそこで null 参照エラーになるので「壊れた」ではなく Pro 機能だと分かる。**コード内容から「HTML に関係あるか」を判定して出し分ける案は不採用**（文字列中の `document`・`eval`・変数経由の間接参照で誤判定が避けられず、動く/動かないが予測不能になるうえ、判定を外した側に倒れると可視プレビューが漏れる）。

**デッキ土台の複数持ち（044: HTML/CSS 土台 ／ 045: SQL 初期化・Pro 限定）**：デッキは**名前付きの土台を複数**持てる（`decks.htmlStages` / `decks.sqlStages`＝ともに `DeckStage[] = {id,name,content}` の JSON 列）。コードブロックは `CodeBlock.deckStageId` / `deckSqlStageId` で1つを選ぶ。**HTML と SQL は同じ型・同じ部品・同じ規則**で、違うのは中身の言語と呼び名だけ（HTML＝「土台」／SQL＝「初期化」。実物の語彙が違うので片方に寄せない。文言キーの対応表は `lib/deckStageLabels.ts`）。⚠️ `DeckStage` の中身のキーは 044 初期実装では `html` だったが 045 で `content` に統一した＝**旧キーの吸収は `parseDeckStages` だけが知っている**（保存し直すと新キーに移る）。**読み取りの正は `htmlStages`/`sqlStages` だけ**で、旧 `decks.htmlInit`/`decks.sqlInit` は**先頭土台のミラー**として残してある（旧バージョンのアプリは旧列しか読まない＝iCloud は DB ファイルごと往復するため。書き戻しは `legacyInitMirror()`）。旧データの吸収は **`toDeck` の `normalizeDeckStages()` に閉じている**（JSON が空なら旧列から1件合成・合成した土台の id は定数 `LEGACY_STAGE_ID`＝毎回 `generateId()` すると選択 id の照合と React の key が揺れる）ので、**画面・コンポーネントは配列だけを見ればよい**。土台の解決は `lib/deckStages.ts` の **`resolveDeckStageHtml` / `resolveDeckStageSql`**（中身は共通の `resolveStage`：「使わない」フラグ→積まない／id 未指定→**先頭**＝044/045 以前の挙動／**解決できない id（削除された土台）→積まない**）。⚠️ **解決できないときに先頭へフォールバックしてはいけない**＝別の土台が黙って適用されて出題の前提が変わるため。土台なしなら js/ts はコンソール実行に戻り、html は本文だけ、SQL は初期化なしの素の DB になるので作者が気づける。UI は デッキ編集の行／`H`（HTML）・`Q`（SQL）キー →`DeckStagesModal`（**HTML/SQL 共用**・`kind` prop で文言を切替。一覧＝追加/削除/1行プレビュー・先頭に「既定」バッジ・行タップで `SqlInitModal` が開き**名前はそのタイトル欄で編集**。043 の画像ライブラリは `listFooter` prop で HTML のときだけ差す＝**一覧の末尾だけ**に出し、**編集面には出さない**）。⚠️ **画像ライブラリを編集面に置いてはいけない**（2026-08-13 に撤去）：①土台が0件だと編集面を開けず到達手段がゼロになるのに、デッキ編集の行は画像があれば点灯するため「設定済みなのに開くと空」に見える ②iPhone でキーボードを出したまま展開するとシート（約345pt）を食い尽くして土台の入力欄が1行に潰れる。同じ理由で**行のラベルは土台0件＋画像ありのとき枚数を出す**（`deck.htmlImagesOnly`）＝「設定済み」だけだと中身が分からない。ブロック側は **`components/editor/DeckStagePicker.tsx`（HTML/SQL 共用）**＝土台が1つなら従来の ON/OFF トグルのまま、2つ以上のときだけ「使わない＋各土台」のチップ選択に変わる（＝土台を1つしか使わないデッキは 044/045 以前と見た目も操作も同じ）。⚠️ トグルの `value` は「使わない」フラグではなく**解決結果**（`activeStageId !== null`）を見る＝削除済み id を指したままのブロックを ON と表示しないため。**中身が空の土台は保存しない**。**Pro ゲートは HTML と SQL で非対称**：HTML は非 Pro なら土台を積まない処理が要る（JS 実行が無料なので積むと可視プレビューが漏れる）が、**SQL は言語自体が `PRO_LANGUAGES`＝実行ゲートで止まる**ので `sqlInits` の組み立てに `isPro` は不要。TSV は往復しない（`inspectTsvExport` が `deckHtmlStages`/`deckSqlStages` の件数を数えて警告に出す）。⚠️ **`updateDeck` は土台の配列も旧列のミラーも「渡されたときだけ」書く**（旧列を無条件に `?? null` で書くと、土台を渡さない更新でミラーだけ NULL になり、新バージョンでは気づけないまま旧バージョン／旧エクスポートから土台が消える）。詳細は `docs/044`（HTML 土台・設計判断の出どころ）と `docs/045`（SQL 初期化・共用化の判断）。

**全画面インタラクティブプレビュー（041・Pro 限定）**：040 のインライン可視 WebView は `pointerEvents="none"` の表示専用で `addEventListener` が発火しない。041 は `ExecutionOutput` のプレビューバーに ⛶ ボタン（`onExpand`）を出し、`InteractivePreviewModal`（`components/code/`）＝ **`presentation="fullScreen"` の RN Modal（別ネイティブ VC）** で操作可能な WebView を開く。別 VC ゆえカードの ScrollView/FlipCard・編集の `NestableDraggableFlatList` と**ジェスチャー競合しない**（これがインライン操作化を採らずモーダルにした理由）。実行系は `buildInteractiveWebSandboxHtml()`＝完了判定・5秒タイムアウトを持たず生きたまま動き、console・未捕捉例外を**1行ずつ逐次 postMessage**（イベントで出たログをライブ表示）。モーダルは下部にライブ console＋ヘッダーに `✕ ｜ タイトル ｜ ⟲ ▶`（**▶＝本文を実行／実行中に押せば初期状態から再実行**、**⟲＝実行前＝土台のみへ戻す**。いずれも `nonce` 再マウント。⟲ は戻り先がある `ran && (土台 or previewBody あり)` のときだけ表示。**▶ は実行前だけ塗りボタン（`#1976D2`）で実行後はゴースト**＝実行前の「押しても無反応な土台」で次の行動を示すため。「実行前」バッジは `<title>` 行を侵すので不採用）。TS はモーダル内で sucrase 変換。学習/編集の両画面（`CodeRunnerView`/`CodeBlockItem`）で描画し、`isPro` かつ Web プレビュー対象（html／js・ts＋土台）のときのみボタン表示。**全画面もインラインと同じ「実行前／実行後」の軸に乗る**（2026-07-31 修正）：呼び出し元が `body`（実行後にライブ実行する本文）・`previewBody`（実行前に土台の後ろへ描く本文＝html の `previewInit` ON のときだけ非空＝インラインの `staticBody` と同ルール）・`initialRan`（＝`execActive = previewMode && !!htmlSource`）を渡し、モーダルが `ran` state で往復する。旧実装は常に本文を渡していたため ⛶ がいつでも実行後相当になり、040 の「実行するまで本文の結果は見せない（答えを先に見せない）」原則が全画面だけ素通しだった。⛶ ボタンはプレビュー枠の中にあり枠は土台/`staticBody` があるときしか出ないので、未実行の全画面が真っ白になることはない。**背後キー抑止**：モーダルは別 VC でもアプリの `UIKeyCommand` は生きるため、`InteractivePreviewContext`（`setOpen`）で開閉を親へ伝え、学習は `session.tsx` の main キー active に `&& !interactivePreviewOpen`＋常時 Esc ハンドラ先頭で `if (interactivePreviewOpen) return;`（**Esc→safeBack でセッションごと抜ける暴発を防止**・閉じるはモーダル自身の Esc）、編集は `BlockEditor` の main/Esc 両 active に `&& !interactivePreviewOpen`（`suspendKeys` と同型で main＋Esc を一括解除）。開くのは ⛶ タッチのみ（開くキーは未割当）、閉じるは Esc／✕。

**実行の締切（console・040 インライン共通）**：`sandbox.ts` の `DEADLINE_SCRIPT` が締切を管理する。既定は `EXEC_TIMEOUT_BASE_MS`（5秒）だが、**ユーザーコードが `setTimeout(fn, 8000)` のように先の発火を予約したら「発火予定＋300ms」まで締切を押し出す**（`extendDeadline`）。絶対上限は `EXEC_TIMEOUT_MAX_MS`（30秒・Python の Pyodide 全体／SQL／C++ の Wandbox と同値）で、**基準は実行開始時刻**なので再帰 `setTimeout` でも 30 秒で頭打ちになる。**`setInterval` では延長しない**（終わりが無いため。clear しなければ従来どおり打ち切り）。何も予約しないコードの体感は不変。固定5秒だと `setTimeout` の学習カードが必ずタイムアウトになるため 2026-08 に導入した。timeout 通知には**実際に適用した上限 `limitMs`** を載せ、`ExecutionOutput` が `code.timeout` / `code.timeoutMessage` に `{{seconds}}` として差し込む（未指定なら5秒扱い）。**タイムアウト系を追加するときは `limitMs` も一緒に送ること**（送らないと「5秒」と表示される）。なお **`while(true)` のような同期無限ループはこの締切では止まらない**（WebView の JS スレッドを占有しタイマー自体が発火しない）。041 全画面は「生きたまま」動かすので締切自体を持たない。

**完了が届かないときの見張り（`useCodeExecution` の watchdog）**：上の締切は「その WebView の JS が生きている」ことが前提なので、①カードが `location.href = ...` でページごと遷移した（harness ごと消える）②同期無限ループでタイマーが発火できない ③コンテンツプロセスが落ちた／`window.ReactNativeWebView` を壊された、のいずれでも**誰も実行を終わらせられず `status='running'` のまま固着する**（コードブロックのヘッダーが緑のまま・スピナーが回り続ける）。そこで RN 側にも見張りタイマーを置く。待ち時間は `watchdogMsFor(language)`＝**web/console 系は既定5秒＋3秒（`WATCHDOG_MARGIN_MS`）、python/sql は 30 秒＋3 秒**（CDN 読み込みを含む正当な上限のため）、**C++ は対象外**（`runCppViaWandbox` の `AbortController` が既に担当）。カードが `setTimeout` で締切を延ばしたときは `DEADLINE_SCRIPT` の `postDeadline` が `{type:'deadline', remainingMs}` を送り、`handleMessage` が見張りを張り直す＝**通知しないと RN 側は延長を知りようがなく、安全側に倒すと全カードが常に30秒待つことになる**。`alert`/`confirm`/`prompt` は**表示直前に絶対上限ぶんの猶予を送っておく**（スレッドが止まっている間は何も送れないため。30秒以上開いたままにすると一度「応答なし」が出るが、閉じれば本来の結果で置き換わる）。発火時は `code.noResponse` を出し、途中まで届いていた `liveLogs` を結果へ引き継ぎ、**可視プレビューの WebView は残す**（遷移先や描画結果を消さない）／console 専用言語の隠し WebView は破棄する（暴走ループが CPU を掴み続けないように）＝完了時と同じ扱い。⚠️ **実行系の言語を追加したら `watchdogMsFor` にも足すこと**（既定は web/console 系と同じ8秒扱いなので、CDN 読み込みなど時間の掛かる実行系を足すと誤って打ち切られる）。

**`alert`/`confirm`/`prompt` とサンドボックス（`DIALOG_SCRIPT`）**：これらはネイティブダイアログとして動作し、**閉じるまで WebView の JS スレッドを止める**。素のままだと ①直前の `console.log` が送信されないまま止まり OK まで何も出ない ②操作待ちの時間が実行時間として締切に算入されタイムアウトする、の2つが起きる。`DIALOG_SCRIPT` が3関数をラップし、**表示直前に `flushLogs()`／閉じた後に `skipDeadline(止まっていた時間)`**（`_startedAt` ごと後ろへずらして再武装）することで両方を防ぐ。戻り値はそのまま返すので `confirm`/`prompt` の挙動は不変。041 全画面は締切を持たないため対象外。

**実行中ログの逐次表示（console・040 インライン共通）**：ログは終了時の1通（`finish` の `logs`）が正だが、それだけだと `setInterval(fn, 1000)` のような「時間をかけて出力するコード」の途中経過が見えない（締切に達してから一気に出る）。`sandbox.ts` の `LOG_STREAM_SCRIPT` が `console.*` のたびに **溜まっていなければ即送信し、連続するぶんだけ 100ms でまとめて `{type:'logs', entries}` で送る**（リーディングエッジ＋トレーリング）（`useCodeExecution` が `liveLogs` に溜め、`ExecutionOutput` が実行中だけ表示。`result` には入れない＝`result` の変化は `onResult`＝学習画面の結果へのスクロールを呼ぶため）。**逐次送信は `LOG_STREAM_MAX`（200件）まで**で、超過分は終了時の1通に入る＝最終的な表示内容は不変。**送信予約は `_origSetTimeout`（ラップ前）で行う**（ラップ後だと保留タイマーに数えられ完了判定が遅れ、締切まで延ばしてしまう）。**先頭を即送りするのは、JS スレッドが止まる状況でも「止まる前に出したログ」を届けるため**：`setTimeout(() => alert('x'), 0); console.log('a');` は console.log が先に走るのに、トレーリングだけだと送信予約が走る前に `alert` がスレッドを止め、OK を押すまで何も出ない＝同期実行に見える（同期無限ループの直前のログも同じ）。即完了するコードでは「先頭1件が逐次＋残りは終了時の1通」になるので、実行中パネルが一瞬出てから結果パネルに置き換わる（内容は同じ。エラー/タイムアウトのカードだけ白→赤の入れ替わりが一瞬見える）。UI は**最初のログが届いてから枠を出す**方式（案A）で、ログを出さないカード（プレビューだけの html/css 等）の実行中の見た目は変えない。

**サンドボックスで使えない/使える API（JS・web 系／console・040 インライン・041 全画面 共通）**：無効化＝呼ぶと TypeError なのは `fetch`・`XMLHttpRequest`・`WebSocket`・`window.open`（通信とウィンドウ生成を遮断）。`window.close()` はエラーにならないが**無反応**（開いた窓が無い）。WebView の `baseUrl` が `about:blank`（オリジンなし＝opaque origin）のため **Storage 系は使用不可**：`localStorage`/`sessionStorage`/`indexedDB` は SecurityError「The operation is insecure.」（`localStorage` は実測確認済み）、`document.cookie` は例外は出ないが保存されない。`import`/`export`・`require()`・npm・動的 `import()` も不可（バンドラ/通信なし）。同じ理由で **`isSecureContext === false`（実測確認済み）＝セキュアコンテキスト限定 API は一律不可**：`navigator.clipboard`・`crypto.subtle`・`crypto.randomUUID`（いずれも `undefined`）・Service Worker 登録・`Notification`・`getUserMedia`・Geolocation。乱数は**セキュアコンテキスト限定ではない `crypto.getRandomValues()` が使える**（UUID が要るならこれで自作する）。**`history.pushState` は `'#x'` のようなハッシュ変更なら通る（実測 OK）**が、`about:blank` は相対パス解決ができないためパス変更（`'/foo'`）は落ちる想定＝SPA ルーティングのデモは不可。**ページ遷移（`<a href>` のタップ・`<form>` の送信・`location.href = ...`）は止めていない＝ブラウザと同じに動く**：041 の全画面プレビューでリンクやフォームから実サイトへ飛べるのは**元からある挙動で教材として使われている**。遷移すると当然サンドボックス文書（土台・本文・console キャプチャ・postMessage ハンドラ）は差し替わるので、戻るには全画面ヘッダーの ▶（WebView を nonce で再マウント）か ✕、インラインなら ⟲ を使う。**唯一の例外が「サンドボックス文書自身への送信」**（`action=""`/`action="#"`）で、これだけは `hooks/useSandboxReload.ts` が**リロードに変換する**：ブラウザではこの送信は同じ URL の再取得＝リロードになり入力欄が空のページが描き直されるが、about:blank には再取得できる元が無いので素通しすると**真っ白になって戻ってこない**。そこで遷移をキャンセルして WebView を作り直す＝同じ HTML が最初から描き直され、**ブラウザと同じ見え方**（入力欄が空に戻る）になる。判定は **`navigationType`（`formsubmit`/`formresubmit`/`reload`）で行い、URL だけで判定してはいけない**（`<iframe>`〈src 無し〉の about:blank 読み込みや初回ロードまで拾ってリロードが暴走する）。`href="#sec"` は `click` なので対象外＝従来どおりスクロールする。`onload` で自動 submit するカード対策に連続リロードは 500ms 間隔で間引く。キャンセルしてもエラー画面は出ない（`RNCWebViewImpl` が `NSURLErrorCancelled` と `WebKitErrorDomain` 101/102 を無視するため）。⚠️ **2026-08-11 に「送信を preventDefault してコンソールに内容を出すガード」を実装したが差し戻した**（ブラウザと違う挙動になるため）。**再実装しないこと。**⚠️ **URL は絶対 URL（`https://…`）で書く**：`href="apple.com"` のようなスキーム無しは**相対 URL** で、ベースが `about:blank` だと解決できない＝**遷移が始まらず何も起きない**（`history.pushState('/foo')` が落ちるのと同じ根。ブラウザで「Cannot GET /apple.com」が出るのは開発サーバーの 404 本文であって、サーバーの無いこのプレビューでは再現できない）。⚠️ **`react-native-webview` は 13.16.1 以上が必須**：13.15.0（Expo SDK 54 のピン）の New Architecture 実装は `onShouldStartLoadWithRequest` イベントで `mainDocumentURL` が nil のとき `std::string(NULL)` を作り**アプリごとクラッシュしていた**。nil になるのは上記の解決不能な相対 URL への遷移＝**分かれ目は form/a ではなくスキームの有無**（`href="apple.com"`・`action="apple.com"` は落ち、`https://apple.com` は落ちずに遷移した＝実機確認済み）。上流の `nullSafeString()` 導入（13.16.1）で根治済み。Expo のピンから外れる警告は `package.json` の `expo.install.exclude` で止めてある＝**開発時のターミナル警告だけでアプリ利用者には無関係**。一方 **`alert()`/`confirm()`/`prompt()` はネイティブダイアログとして動作する**（react-native-webview の WKUIDelegate 既定・特別な実装なし。`confirm`/`prompt` は同期ブロックのため console・040 では実行の締切に掛かりうる／041 全画面は上限なし）。localStorage を使えるようにする案（メモリ内 shim／`incognito`／実在オリジン付与）は 2026-07 に検討したが、非永続で誤解を生む・「毎回まっさら／カードごとに独立」の実行モデルと衝突するため**見送り**（この記述が結論＝再提案しない）。

**HTML 画像ライブラリ（043・Pro 限定）**：デッキに写真を登録し、HTML ブロック／HTML 土台から **`<img src="img://名前">`**（CSS の `url(img://名前)` でも可）で参照する。`decks.htmlImages`（`DeckImage[] = {name, uri}` の JSON 列）に**参照名とパスだけ**を保存し、画像の実体は画像ブロックと同じ `images/`（`local://images/xxx`）に置く。**base64 は実行の瞬間にメモリ上で作るだけで DB には入らない**（＝エクスポート・iCloud 同期・エディタの入力欄が膨らまない。これが data URI 直貼りに対する本方式の存在理由）。解決は `lib/htmlImages.ts` の `resolveHtmlImageRefs()` が **合成後の最終 HTML に対して1回**行い、`sandbox.ts` は無改造（`img://` は土台にも本文にも書けるため最終形に1回かければ両方拾える）。差し込み口は3つ＝`useCodeExecution.run()`（実行）・`ExecutionOutput`（実行前プレビュー・**400msデバウンスの後**で解決）・`InteractivePreviewModal`（041・**解決待ちの間は WebView を出さない**＝未解決 HTML を描画して差し替えるとスクリプトが2回走るため）。`hasImageRefs()` の同期判定で **`img://` を含まないカードは従来どおり完全に同期**（挙動・速度とも不変）、かつ **web 系限定**（python/sql 本文の `img://` は書き換えない）。非同期化に伴い `runSeqRef` の通し番号で「解決待ちの間に次の実行/clear が走ったら古い結果を捨てる」。登録は `pickAndSaveImage({ maxDimension })`＝長辺1024へ縮小＋png/jpg へ正規化（HEIC 対策・MIME 判定を2択に固定）。UI は `HtmlImageLibrary`（`DeckStagesModal` の `listFooter` prop＝**土台一覧の末尾だけ**に描画する・既定は折りたたみ）。⚠️ **編集面（`SqlInitModal`）には置かない**（2026-08-13 に撤去。`footer` prop ごと削除済み）：①土台が0件だと編集面を開けず到達手段がゼロになる ②iPhone でキーボードを出したまま展開するとシート（約345pt）を食い尽くして土台の入力欄が1行に潰れる。画像はデッキ単位のデータなので、粒度の合う土台一覧が置き場所として正しい。タグが要るときは一覧でコピーしてから編集面を開く（土台テキストはライブ反映なので往復しても失われない）。**削除は配列から外すだけでファイルは消さない**（保存前キャンセルで参照が生き残るため。孤児は起動時掃除が回収）。解決できない参照は**プレースホルダ SVG**（`img://name` を描画）になる。

**HTML ブロック／HTML 土台で書けるもの・書いても効かないもの**：`buildWebSandboxHtml`/`buildInteractiveWebSandboxHtml`/`buildStaticPreviewHtml` はいずれも **土台（`${stages}`）とブロック本文（`${markup}`）を `<body>` 直下へ文字列連結**する構造で、`<head>` は固定（charset・viewport・遮断スクリプト）＝**ユーザーが head に足す手段は無い**。そのため head 系は「無視される」のではなく「置き場所が悪くて効果がない」：`<meta charset>`（エンコーディング確定済み）・`<meta name="description">`/`og:*`/`keywords`/`robots`・`<meta name="theme-color">`・`<link rel="icon"/"manifest"/"canonical"/"preconnect">` はすべて無効果。**`<title>` だけは例外で 041 の全画面ヘッダーに表示される**（`buildInteractiveWebSandboxHtml` が `DOMContentLoaded`/`load` で `{type:'title'}` を送り、モーダルが `pageTitle || LANG_LABELS[language]` で出す。body に落ちても `document.title` が拾うことを実測確認済み。`document.title = ...` の書き換えも反映。**インラインのプレビュー枠には出さない**＝ブラウザもページ内に描画しないため誤解を招く。全画面は web 系でしか開けないので他言語との不揃いも起きない）。`<!DOCTYPE>` と `<head>` 開始タグは本文中では無視され、`<meta name="viewport">` は head 側で指定済み（後勝ちで効く可能性はあるので書かない）。ただし **`<html lang>`/`<body style>` の属性は既存要素にマージされて実際に効く**（HTML 仕様の挙動）。フル HTML ドキュメントを丸ごと貼っても「head の中身が body に落ちるだけ」でだいたい動く。**画像は `<img src="data:...">` とインライン `<svg>`（＋CSS の `url(data:...)`）なら通信もオリジンも不要で確実**。相対パス・ローカルファイル・アプリ内の画像ブロックの画像は `baseUrl='about:blank'`＋`allowFileAccess` 未設定のため**不可**（ただし**端末内の写真を使いたいときは 043 の画像ライブラリ＝`img://名前` を使う**。実行時に data URI へ置換されるので上の制約に触れない）。**外部 https の `<img>` は実測で表示された**（CSP 無し・`originWhitelist={['*']}`）＝`<script src>`/`<link rel=stylesheet>`/`<iframe src>` も同経路で通るが、**オフラインで無音で壊れる**ので教材では非推奨（`@font-face` の Web フォントだけは CORS 必須＝`Origin: null` を許す CDN のみ）。**両プレビューで等しく無意味**なのは `:hover`/`:focus-visible`（指にホバーが無い）・**`title` 属性のツールチップ**（`<abbr title>`・`<span title>` 等。**iOS/iPadOS の WebKit がツールチップ UI 自体を実装していない**＝ホバーがある macOS 専用の機能。アプリ側は属性を一切落としておらず `el.title` で値は取れるので、原因を DOM 側に探さないこと。041 全画面でも同じ。教材で見せたいときは **`content: attr(title)` で自作する**＝`:hover` の代わりに `:active`/`:focus`〈`abbr` は既定でフォーカス不可なので `tabindex="0"` が要る〉をトリガにすれば全画面でタップ表示でき、常時見せるなら `abbr[title]::after{content:" (" attr(title) ")"}` でよい。この土台をブロック固有土台に置けば「本文には `<abbr>` のマークアップだけ書かせる」出題になる）・**`<select>` の `size` によるリストボックス表示**（iOS/iPadOS の WebKit は `<select>` を常にネイティブのピッカーとして描画し `size` を無視する。**デスクトップの Safari では効く**＝プレビュー固有の制限ではなく **iPhone/iPad の Safari で同じ HTML を開いても同じ**。`title` 属性と同じく**アプリは属性を落としていない**ので `el.size` で値は取れる＝原因を HTML 側に探さないこと。`multiple` も見た目はピッカーのままだが複数選択そのものは機能する。リストボックスの見た目を見せたいときは**ラジオ/チェックボックスの縦並びを `overflow-y:auto` の箱に入れ `input:checked + label` で選択色を付ける**のが定番の代替）・`<noscript>`（JS 常時有効）・`target="_blank"`（`window.open` 無効・別窓を開けない）・`<base target>`・`<video>`/`<audio>`（`allowsInlineMediaPlayback={false}`＋要ユーザー操作＋ソースが無い＝実質使えない）。**040 インライン特有**の制限は `scrollEnabled={false}`・`pointerEvents="none"`＝**操作不可**と**横方向のはみ出しが切れる**こと（041 全画面では解消）。**縦は中身の高さに自動追従する**：`HEIGHT_REPORT_SCRIPT`（`sandbox.ts`・`buildWebSandboxHtml` と `buildStaticPreviewHtml` に注入）が `{type:'previewHeight'}` を postMessage し、`ExecutionOutput` が実行結果ハンドラへ渡す前に横取りして箱の高さを 220〜画面高6割にクランプする。はみ出す分は外側のカードの ScrollView で読む。**内側をスクロール可能にしてはいけない**（タッチを取ると 041 を全画面にした理由＝ジェスチャー競合を踏む）。通知は連続追従せず `DOMContentLoaded`/`load`/`load+300ms` の3回だけ（編集リストで高さが動き続けると下ブロックの `onLayout` がずれる）、かつ **harness より前に置いて生の `setTimeout` を捕まえる**（後ろだと保留タイマーに数えられ完了判定が遅れる）。**土台の `<script>` 内の `console.log` は静的プレビュー（未実行時）では console キャプチャが無いのでどこにも出ない**（実行後は出る）。

### 実装上の注意点

- **「オンに見えるのに効いていない」状態を作らない（トグル・通知・条件つき機能の鉄則）**: 機能を足すときは**オフ側・無効側で何が起きるか**を最初に確認する。エラーが出ないぶん最も気づきにくい壊れ方で、046 だけで4件踏んだ（目標 OFF で未達成リマインダーが黙って止まる／確認ダイアログの余白タップが壊れた状態に着地する／アプリ起動中に通知が消費されて消える／大元の通知トグル OFF でスケジュールが効かない）。対処は2つだけ＝**そもそもその状態を作らせない**（例：目標 OFF 時に2択で決めさせる・閉じる＝完全キャンセル）か、**「なぜ効かないか」を画面に出す**（例：注意行＋`opacity: 0.55` で淡く。状態ではなく**結果**を書く＝「オフです」ではなく「動作しません」）。
...
- **削除時の後始末（042 で修正）**: `deleteDeck` は `deleteDecksBulk(db, [id])` に委譲し、**`grade_logs` の削除と画像ファイルの削除**を含む（旧実装は両方とも取りこぼして孤児データが溜まっていた）。`deleteCard`/`deleteCardsBulk` も `grade_logs` を削除する。`deleteCardsBulk` の第3引数は `deckId | deckIds[]` で、複数デッキのカードを混ぜて削除しても `cardCount` を全対象デッキ分数え直す。**カード削除系を追加するときは `card_contents`/`card_tags`/`reviews`/`review_logs`/`grade_logs`＋画像の6点セットを必ず消すこと**（`foreign_keys` pragma 未設定のため）。デッキ削除は配下カードの画像に加えて **`decks.htmlImages`（043 のライブラリ画像）も消す**（`deleteImagesOfDecks`）。
- **画像の「参照集計」は `lib/image.ts` の `getReferencedImageFilenames()` が唯一の定義元（重要）**: この関数が返さないファイルは、起動時の `cleanupOrphanImages()`（`app/_layout.tsx`）に**削除され**、`lib/sync/syncEngine.ts` の3箇所（上りアップロード・リモート整理・下りダウンロード）で**iCloud 同期の対象からも外れる**。現在の参照元は2系統＝①`card_contents` の image ブロック ②`decks.htmlImages`（043）。**画像の新しい持ち方を増やしたら必ずここに足すこと**（足し忘れると「登録した画像が次回起動で消える」「同期されない」が同時に起きる）。引数はテーブル名ではなく**スキーマ接頭辞**（`''` / `'bkimg.'`）で、ATTACH した自動バックアップDB（029）も同じ関数で走査する。**旧スキーマのバックアップには `htmlImages` 列が無い**ため `decks` の SELECT だけ try/catch で握り潰し、カード側の集計は活かす。なお `lib/export.ts` の `extractImageFilenames()` は別実装なので**両方直す**（漏れるとエクスポートに画像本体が入らない）。
...
- **`patches/react-native-key-command+1.0.15.patch` は必須（重要）**：本家 `HardwareShortcut.m` の `matchesInput:` が文字列を `==`（**ポインタ比較**）で照合しており、`unregisterKeyCommand` が対象を見つけられず**登録が残り続ける**／`handleKeyCommand:` の照合も外れて**ハンドラが発火しない**。短い ASCII（`j`・`\r` 等）は tagged pointer になり値が等しければポインタも等しいため素通りするが、**`UIKeyInputUpArrow`（17文字）のような長い定数はヒープ確保＝JS から渡すたびに別インスタンス**になるため壊れる。症状は「**文字キーのショートカットは全部効くのに、テキスト/コードブロックの中で矢印と Return だけ効かなくなり、アプリを再起動するまで直らない**」（iPad の Split View でアプリを行き来すると誘発されやすい＝UIKit が keyCommands を作り直して文字列インスタンスが入れ替わるため）。iPad では残った矢印登録に `wantsPriorityOverSystemBehavior` が付くので入力欄のカーソル移動を奪い続ける。パッチは `isEqualToString:` に直すだけ。**ライブラリを上げたらこのパッチの当たり直しを必ず確認する**（同パッチには PageUp/PageDown/Home/End の定数追加も含む）。
...
- **デッキ新規作成キー**: N = デッキ名にカーソル、M = 説明欄にカーソル（Memo/Message。N と隣接）、C = カラー順送り（青→プリセット→テーマ色→白黒の循環。Shift+C で逆順）、I = アイコン選択を開く、H = HTML/CSS 土台の一覧を開く（Pro時）、Q = SQL初期化の一覧を開く（Pro時。**画面の行順・ショートカット一覧とも HTML/CSS 土台 → SQL 初期化**＝土台を使う言語が html/css/js/ts の4つで SQL より頻度が高いため。キー割り当ては頭文字由来なので行順とは独立）、U/D・PgUp/PgDn = 画面スクロール（上/下・段階）、Home/End = 最上部/最下部、S = 保存、X = 閉じる、Esc = 編集中→カーソル解除／非編集→閉じる。デッキ名 Return で説明欄へ移動。Tab/矢印は不使用（iPad フォーカスエンジン対策＝文字キー＋住み分けで完結）
...
- **土台の編集面キー**（`SqlInitModal`＝HTML 土台／SQL 初期化の1件分の編集面）: Esc = 閉じる（確定＝ライブ値保持）。全面テキストエディタのため他の文字キーは入力に消費される
- **土台一覧モーダルキー**（`DeckStagesModal`＝HTML/SQL 共用・048）: J/K = 土台のフォーカス移動（ヌルサイクル・ID 追跡）、Return = フォーカス中の土台を開く、Delete = 削除（確認あり）、N = 土台を追加、Esc = 階層ディスマス（削除確認 → 編集面は自前の Esc → **フォーカス解除** → 一覧を閉じる）。親のデッキ編集画面は `subModalOpen()` で抑止し、**この一覧のキーは編集面/削除確認が上に乗っている間 `active` で解除する**（そちらが最上位になるため）。自動スクロールは `useListNavigation` の `scrollToIndex`（FlatList 用）が使えないので、行の `onLayout` で Y を控えて `scrollTo` する。⚠️ **`S`（保存）は割り当てない**：このシートの ✓ は `onClose` を呼ぶだけで保存はデッキ編集画面の役目なので、Esc と同じ動作になり意味がない。⚠️ **画像ライブラリのキー操作は未実装**（048 Phase 2 として保留。理由と不採用案は `docs/048`）
...
- **ネイティブ登録は `useKeyCommands` の参照カウントが管理する（重要）**: `react-native-key-command` の `HardwareShortcuts.m` は**「誰が登録したか」を持たない**。`registerKeyCommand:` は毎回 `_commands` に**追加**し（同じキーでも重複して残る＝**登録数だけイベントを発行＝1押下で複数回発火**）、`unregisterKeyCommand:` は**入力が一致する全エントリを消す**（＝**別のコンポーネントが登録した同じキーまで巻き添えで消える**）。そこで `lib/useKeyCommands.ts` の `acquireKeyCommands`/`releaseKeyCommands` が **(input, 修飾) ごとの利用者数を数え、0→1 でだけ登録・1→0 でだけ解除**する。**ネイティブへ直接 register/unregister しないこと**（このフックが唯一の経路）。048 で両方の症状を実際に踏んだ：①`IconPickerModal` が `active` 無しで J/K を登録していたため、土台一覧に J/K を足した瞬間に二重発火（3件なら1押下で真ん中へ飛ぶ）②`active` を足したら今度は閉じるときの解除が親画面の H/N/Esc まで消した
- **`useKeyCommands` の第2引数（`active`）は「今そのキーを担当するのは誰か」を決める**: RN `<Modal>` は `visible=false` でも children がマウントされるので、常時マウントのモーダルは非表示のあいだ `active=false` にして、**見えている側だけがハンドラを持つ**状態にする。ハンドラ内の `if (visible)` だけだと非表示側もイベントを受けて担当が曖昧になる（`if (visible)` は残したうえで第2引数にも渡す）
...
- **`react-native-draggable-flatlist` のドラッグ終了リセット（patch 済み・重要）**: 本家は `activeIndexAnim` を `reset()` でしか -1 に戻さず、その `reset()` は**データの並び順が変わったときしか呼ばれない**。そのため「同じ位置に戻して離す」ドラッグの後は `activeIndexAnim` が 0 以上のまま残り、`useAutoScroll` の「ドラッグ開始」検知（-1 → 0以上 の遷移）が二度と発火しなくなる＝`scrollTarget` が古いまま固定され、**以後のドラッグで端までドラッグしてもオートスクロールしない**（スクロール位置を変えた後ほど顕著）。`patches/react-native-draggable-flatlist+4.0.3.patch` で `onDragEnd` 後に必ず `reset()` を予約するよう修正済み。かつてはロック切替のたびにリストごと再マウントされ全状態が初期化されていたため露見しなかった。ライブラリを上げたらこのパッチの当たり直しを確認する。
