...
- [ ] **Base64埋め込み対応**: エクスポート時に画像ファイルを読み込みBase64文字列としてJSONに含める。インポート時はBase64をデコードして `images/` ディレクトリにファイルとして書き出す。
- [ ] **画像含む/含まない選択UI**: エクスポート実行時に「画像を含める（完全バックアップ）／含めない（軽量バックアップ）」を選択できるようにする。「含めない」を選んだ場合は別端末では画像が表示されない旨を警告する。
- [ ] **大容量警告**: 画像を含めてエクスポートする際、推定ファイルサイズが大きい場合（目安: 10MB超）に確認ダイアログを表示する。
