...
- [x] オフライン2端末で別デッキ学習→一方が負け→自動バックアップからデッキをマージ→両方の学習が残ることを確認
- [x] 現データに同デッキの新しい学習があるカードは維持される（行単位LWW）ことを確認
- [x] 画像付きカードのデッキで、マージ後に画像が表示されることを確認
- [x] マージ後に学習/統計/ホーム/カード一覧へ即反映（dataRevision）されることを確認
