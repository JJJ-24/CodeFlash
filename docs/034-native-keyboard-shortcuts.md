...
- [x] 各画面で割り当てキーが1回で反応（J/K・Space・Return・`,`/`.`・1–4・各レター）
- [x] **タッチ操作が常に1回で効く**（下部ボタン・タブ・行タップ・戻る/新規作成）
- [x] バックグラウンド→復帰後もフリーズしない（本チケットの主目的）
- [x] テキスト編集中はキーが文字入力として働き、ショートカットが暴発しない
- [x] カード編集の編集⇄非編集モード切替で住み分けが正しい
- [x] `keyboardShortcutsEnabled` OFF で一切介入しない
- [x] 物理キーボード無し（画面のみ）でも全タッチ操作が正常
