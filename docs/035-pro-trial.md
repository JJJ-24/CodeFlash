...
**ステータス:** ✅ 完了（Phase 0〜4 全完了・v1.9.0-dev・実機/シミュレータ確認済み 2026-07-11）
...
- **巻き戻し無効化フラグはローカルのみ（許容・2026-07-11 決定）**：恒久無効化フラグは AsyncStorage のみで KV には保存しないため、**再インストールでペナルティだけは消える**。ただし KV の `trialStartedAt` は残るので新規の7日枠は得られず、悪用しても元の枠の残り時間が戻るだけ（それ以上は時計を狂わせたまま使い続ける必要がある）。設計方針（簡易ガードで十分）どおり許容。
- **KV が使えない状況**（iCloud 未ログイン等）：その端末ではローカル判定にフォールバック（再インストールで再体験の余地は残るが許容）。
- **時計が大きくずれている間のタップ無反応（許容・2026-07-11）**：時計を大幅に操作した状態では、フォアグラウンド復帰時の `refreshTrial()` → KV 読み出しの `synchronize()` が iCloud のエラー/リトライで詰まり、JS スレッドが数秒ブロックされてタップが効かないことがある（実機で観測）。自然回復し、時計異常時のみの現象なので許容（改修するなら KV 読み出しの非同期化）。
...
- [x] 期限切れでテーマ→`default`、FSRS→90% に戻ることを確認（実機確認 2026-07-11：時計+8日でテーマが default 配色に戻ることを確認）
- [x] クラウドデータを削除しないこと・再Pro/再体験なしで同期再開しないことを確認（実機確認 2026-07-11：期限切れで同期停止＆`syncEnabled` が OFF に戻る＝再Pro後に同期画面を開くとトグルOFFで要手動再ON。再ONでクラウドデータが残ったまま同期再開）
...
- [x] 開始→7日経過（時計操作）で全Pro機能が無料へ戻ることを実機確認（2026-07-11：体験済み表示・テーマ default・同期OFF・設定/統計の Pro ゲートすべて無料状態へ復帰）
- [x] 再インストール後に再体験できない（iCloud KV）ことを実機確認（2026-07-11）：自然期限切れの記録（過去日付で開始→自動設定に戻し期限切れ化）が再インストール後も「体験済み」のまま維持されることを確認。※体験中の記録も startedAt ごと復元される（新規の7日枠にはならない）。巻き戻し無効化フラグのみローカルのため再インストールで消える（留意点参照・許容）
- [x] 期限切れ→購入でテーマ/FSRS選好値が自動復元されることを確認（2026-07-11：`__DEV__` の purchased トグルで購入をシミュレートし、テーマ・FSRS保持率の選好値復元とクラウドデータ保持を確認）
- [x] iCloud 未ログイン端末でのフォールバック挙動を確認（2026-07-11・シミュレータ：診断で `iCloud=false` を確認 → 体験開始OK（ローカルのみ記録）→ 再起動後も体験中が維持）
