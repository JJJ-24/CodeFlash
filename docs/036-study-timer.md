...
### パイの終端に表示角の下限を敷く（2026-07-31 追記）

**症状**：30分タイマーで、パイが空になって見えてから終了通知まで6〜7秒空く。しかもこの空白はタイマーを
長くするほど長くなる（実機報告）。

**原因は計時ではなく見え方**。パイの角度＝残り割合×360° なので、扇は最後まで残っているのに細くて見えない：

- 扇の幅は **1° あたり外周で約 0.5pt**（3x で約 1.5px）。実測の視認限界は 1.3°前後だった
  （30分＝1800秒で 6〜7秒 → 1.2〜1.4°）。
- **1° の実時間はタイマー長に比例する**（30分なら 5秒、60分なら 10秒）ため、長いタイマーほど
  「消えたのに終わらない」空白が長くなる。
- 計時側は学習 tick が絶対時刻（`endAt = Date.now() + remainingMs`・250ms 判定）、パイも同じ
  `remainingMs` を duration にした線形アニメで、**両者は同じ壁時計上で同時に 0 に着く**。ズレていない。

**対策**：`animatedProps` の中で表示角に下限を敷き、`[0,1]` を `[PIE_MIN_FRAC, 1]` へ線形に写す
（`PIE_MIN_FRAC = 1/90`＝4°）。残りがある間は必ず見える太さの扇が残り、`progress` がちょうど 0 に
なった瞬間＝通知と同じ点で消える＝**「消えた瞬間が終了」**になる。

- **床を張る（`Math.max(p, min)`）方式は不採用**：最後の 1/60 で扇が停滞して見える。線形マップなら
  速度は `(1-min)` 倍でほぼ変わらず、最後に 4° ぶんが一瞬で消えるだけ。
- 副作用＝パイが実際より最大 `PIE_MIN_FRAC` ぶん多く見える（30分なら約20秒）。パイから秒を読む
  使い方はしないので許容。正確な残り時間が要るときは数字表示（`studyTimerTime`）を on にする。
- 学習・休憩の両方に効く（同じコンポーネント）。終了時の満円（blink/アラート）は `p=1` なので不変。
