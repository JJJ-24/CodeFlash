...
### 追追記（2026-07-14）：休憩中の表示を on/off に追従＋円フォールバック

当初は「休憩中は円非表示設定を無視して常にフルリング」だったが（本文 Phase 5・上記「不変」記述）、円=オフでも休憩中に円が出る一方で数字は `time==='on'` のときだけ、という非対称が分かりにくいという指摘を受けて調整。**休憩中もタップ無効＝ピーク不可**という制約（＝必ず1つは残り時間の指標が要る）は保ったまま、次のルールに統一：
- 休憩中の**数字**：`time==='on'` のとき表示（従来どおり）。
