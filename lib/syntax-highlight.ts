...
const KEYWORDS: Record<string, Set<string>> = {
  ...
  sql: new Set([
    ...
    'ON','USING','AS','AND','OR','NOT','IN','IS','NULL','LIKE','GLOB','BETWEEN','ASC','DESC',
    'ORDER','BY','GROUP','HAVING','INSERT','INTO','VALUES','UPDATE','SET','DELETE','CREATE',
    'TABLE','VIEW','TRIGGER','DATABASE','TEMP','TEMPORARY','DROP','ALTER','ADD','COLUMN',
    'RENAME','TO','INDEX','UNIQUE','PRIMARY','KEY','FOREIGN','REFERENCES','CONSTRAINT','CHECK',
    'DEFAULT','AUTOINCREMENT','CASCADE','COLLATE','IF','DISTINCT','LIMIT','OFFSET','UNION',
    'ALL','EXISTS','CASE','WHEN','THEN','ELSE','END','WITH','RECURSIVE',
    'COUNT','SUM','AVG','MIN','MAX',
    'INTEGER','INT','TEXT','REAL','BLOB','NUMERIC','VARCHAR','CHAR','BOOLEAN','DATE','DATETIME',
    'TIMESTAMP','FLOAT','DOUBLE','DECIMAL',
    'select','from','where','join','left','right','inner','outer','full','cross','natural',
    'on','using','as','and','or','not','in','is','null','like','glob','between','asc','desc',
    'order','by','group','having','insert','into','values','update','set','delete','create',
    'table','view','trigger','database','temp','temporary','drop','alter','add','column',
